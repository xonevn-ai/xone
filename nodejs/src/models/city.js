...
const { countrySchema, stateSchema } = require('../utils/commonSchema');
...
schema.post(['findOneAndUpdate', 'updateOne'], async function (doc) {
    if (doc) {
        ...
        if (updatedData)
            await createJob(JOB_TYPE.UPDATE_DBREF, { collectionDetails: COLLECTION_REF_UPDATE.CITY, updatedData: updatedData._doc });
    }
});
...
schema.pre(['findOneAndDelete', 'deleteOne'], async function (doc) {
    if (doc) {
        ...
        if (deleteData)
            await createJob(JOB_TYPE.DELETE_DBREF, { collectionDetails: COLLECTION_REF_UPDATE.CITY, removeData: deleteData._doc });
    }
});

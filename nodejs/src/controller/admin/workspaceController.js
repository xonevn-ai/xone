...
const deleteWorkSpace = catchAsync(async (req, res) => {
    ...
    if (result) {
        res.message = _localize((req.body.isHardDelete) ? 'module.delete' : 'module.archive', req, WORKSPACE);
        return util.successResponse(result, res);
    }
    return util.failureResponse(_localize('module.deleteError', req, WORKSPACE), res);
});

const deleteAllWorkSpace = catchAsync(async (req, res) => {
    const result = await workspaceService.deleteAllWorkSpace(req);
    if (result) {
...
const restoreWorkspace = catchAsync(async (req, res) => {
    const result = await workspaceService.restoreWorkspace(req);
    if (result) {
        res.message = _localize('module.restore', req, WORKSPACE);
        return util.successResponse(result, res);
    }
    return util.failureResponse(_localize('module.notFound', req, WORKSPACE), res);
});
